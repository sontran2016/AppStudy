define(function (require) {
  ...
  var controller = [
    ...
    function ($state,
      ...
      if(vm.userModel.machineIds.length) {
        _.forEach(vm.itemList, function (item) {
          if(_.indexOf(_.map(vm.userModel.machineIds, 'id'), item.id) !== -1) {
            item.selected = true;
            vm.selectedItems.push(item);
          } else {
            item.selected = false;
      ...
      vm.close = close;
      vm.setChoosenMachine = setChoosenMachine;
